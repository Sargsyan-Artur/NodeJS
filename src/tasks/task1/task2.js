...
const writeInJson = async () => {
    await fs.writeFile('./csv/1.1.json', JSON.stringify(Object.assign({}, await task2(csvFilePath))), err => {
        if (err) {
            console.error(err);
        }
    });
...
(async () => {
    ...
    // 1.3
    await writeInTxt(task2, csvFilePath)
})()
...
fromCsvToTxt(task2, csvFilePath)
